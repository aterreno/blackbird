...
namespace Kraken {
    ...
    double getQuote(Parameters &params, bool isBid) {
        ...
        if (isBid) {
            quote = json_string_value(
                    json_array_get(json_object_get(json_object_get(json_object_get(root, "result"), "XXBTZUSD"), "b"),
                                   0));
        } else {
            quote = json_string_value(
                    json_array_get(json_object_get(json_object_get(json_object_get(root, "result"), "XXBTZUSD"), "a"),
                                   0));
        }
        if (quote != NULL) {
    ...
    int sendLongOrder(Parameters &params, std::string direction, double quantity, double price) {
        ...
        *params.logFile << "<Kraken> Trying to send a \"" << direction << "\" limit order: " << quantity << " @ $"
                        << price << "..." << std::endl;
        ...
        std::string options =
                "pair=" + pair + "&type=" + type + "&ordertype=" + ordertype + "&price=" + pricelimit + "&volume=" +
                volume;
    ...
    double getLimitPrice(Parameters &params, double volume, bool isBid) {
        ...
        if (krakenGotLimPrice) {
            ...
            root = json_object_get(json_object_get(
                    getJsonFromUrl(params, "https://api.kraken.com/0/public/Depth", "pair=XXBTZUSD", GETRequest),
                    "result"), "XXBTZUSD");
    ...
    json_t *authRequest(Parameters &params, std::string url, std::string request, std::string options) {
        ...
        SHA256((unsigned char *) payload_for_signature.c_str(), strlen(payload_for_signature.c_str()),
        ...
        hmac_digest = HMAC(EVP_sha512(), decoded_key.c_str(), decoded_key.length(), signature_data.data(),
                           signature_data.size(), NULL, NULL);
        std::string api_sign_header = base64_encode(reinterpret_cast<const unsigned char *>(hmac_digest),
