...
namespace Gemini {
    ...
    int sendLongOrder(Parameters &params, std::string direction, double quantity, double price) {
        *params.logFile << "<Gemini> Trying to send a \"" << direction << "\" limit order: " << quantity << "@$"
                        << price << "..." << std::endl;
        std::ostringstream oss;
        oss << "\"symbol\":\"BTCUSD\", \"amount\":\"" << quantity << "\", \"price\":\"" << price << "\", \"side\":\""
            << direction << "\", \"type\":\"exchange limit\"";
    ...
    double getLimitPrice(Parameters &params, double volume, bool isBid) {
        ...
        json_t *root;
        if (isBid) {
            root = json_object_get(getJsonFromUrl(params, "https://api.gemini.com/v1/book/btcusd", "", GETRequest),
                                   "bids");
        } else {
            root = json_object_get(getJsonFromUrl(params, "https://api.gemini.com/v1/book/btcusd", "", GETRequest),
                                   "asks");
    ...
    json_t *authRequest(Parameters &params, std::string url, std::string request, std::string options) {
        ...
        std::string tmpPayload = base64_encode(reinterpret_cast<const unsigned char *>(oss.str().c_str()),
                                               oss.str().length());
        oss.clear();
        oss.str("");
        ...
        digest = HMAC(EVP_sha384(), params.geminiSecret.c_str(), strlen(params.geminiSecret.c_str()),
                      (unsigned char *) tmpPayload.c_str(), strlen(tmpPayload.c_str()), NULL, NULL);
