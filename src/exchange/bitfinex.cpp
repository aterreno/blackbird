...
namespace Bitfinex {
    ...
    int sendOrder(Parameters &params, std::string direction, double quantity, double price) {
        *params.logFile << "<Bitfinex> Trying to send a \"" << direction << "\" limit order: " << quantity << "@$"
                        << price << "..." << std::endl;
    ...
    double getLimitPrice(Parameters &params, double volume, bool isBid) {
        ...
        bool GETRequest = true;
        if (isBid) {
            root = json_object_get(getJsonFromUrl(params, "https://api.bitfinex.com/v1/book/btcusd", "", GETRequest),
                                   "bids");
        } else {
            root = json_object_get(getJsonFromUrl(params, "https://api.bitfinex.com/v1/book/btcusd", "", GETRequest),
                                   "asks");
    ...
    json_t *authRequest(Parameters &params, std::string url, std::string request, std::string options) {
        ...
        digest = HMAC(EVP_sha384(), params.bitfinexSecret.c_str(), strlen(params.bitfinexSecret.c_str()),
