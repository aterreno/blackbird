...
bool checkEntry(Bitcoin *btcLong, Bitcoin *btcShort, Result &res, Parameters &params) {
    ...
    if (params.verbose) {
        *params.logFile << "   " << btcLong->getExchName() << "/" << btcShort->getExchName() << ":\t"
                        << percToStr(res.spreadIn);
        *params.logFile << " [target " << percToStr(params.spreadEntry) << ", min "
                        << percToStr(res.minSpread[longId][shortId]) << ", max "
                        << percToStr(res.maxSpread[longId][shortId]) << "]";
        if (params.useVolatility) {
            if (res.volatility[longId][shortId].size() >= params.volatilityPeriod) {
                ...
                *params.logFile << "  volat. n/a " << res.volatility[longId][shortId].size() << "<"
                                << params.volatilityPeriod << " ";
            }
        }
        if (res.trailing[longId][shortId] != -1.0) {
            *params.logFile << "   trailing " << percToStr(res.trailing[longId][shortId]) << "  "
                            << res.trailingWaitCount[longId][shortId] << "/" << params.trailingCount;
        }
        ...
        if (res.trailing[longId][shortId] == -1.0) {
            ...
            if (res.spreadIn < res.trailing[longId][shortId]) {
                if (res.trailingWaitCount[longId][shortId] < params.trailingCount) {
                    ...
                    res.exitTarget =
                            res.spreadIn - params.spreadTarget - (2.0 * btcLong->getFees() + 2.0 * btcShort->getFees());
                    res.trailingWaitCount[longId][shortId] = 0;
                    return true;
                }
            } else {
...
bool checkExit(Bitcoin *btcLong, Bitcoin *btcShort, Result &res, Parameters &params, time_t period) {
    ...
    if (params.verbose) {
        *params.logFile << "   " << btcLong->getExchName() << "/" << btcShort->getExchName() << ":\t"
                        << percToStr(res.spreadOut);
        *params.logFile << " [target " << percToStr(res.exitTarget) << ", min "
                        << percToStr(res.minSpread[longId][shortId]) << ", max "
                        << percToStr(res.maxSpread[longId][shortId]) << "]";
        if (params.useVolatility) {
            if (res.volatility[longId][shortId].size() >= params.volatilityPeriod) {
