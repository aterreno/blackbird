...
int main(int argc, char **argv) {
    ...
    csvFile
            << "TRADE_ID,EXCHANGE_LONG,EXHANGE_SHORT,ENTRY_TIME,EXIT_TIME,DURATION,TOTAL_EXPOSURE,BALANCE_BEFORE,BALANCE_AFTER,RETURN\n";
    csvFile.flush();
    // create the log file
    ...
    for (int i = 0; i < numExch; ++i) {
        ...
        if (!params.demoMode) {
            ...
            logFile << balance[i].usd << " USD\t" << std::setprecision(6) << balance[i].btc << std::setprecision(2)
                    << " BTC" << std::endl;
        }
        ...
        if (!inMarket) {
                        ...
                        if (checkEntry(&btcVec[i], &btcVec[j], res, params)) {
                            ...
                            if (!params.useFullCash) {
                                ...
                                if (res.exposure > params.maxExposure) {
                                    logFile << "WARNING: Opportunity found but exposure ($" << res.exposure
                                            << ") above the limit" << std::endl;
                                    logFile << "         Max exposure will be used instead ($" << params.maxExposure
                                            << ")" << std::endl;
            ...
            if (checkExit(&btcVec[res.idExchLong], &btcVec[res.idExchShort], res, params, currTime)) {
                ...
                if (limPriceLong == 0.0 || limPriceShort == 0.0) {
                    ...
                    int longOrderId = 0;
                    int shortOrderId = 0;
                    logFile << std::setprecision(6) << "BTC exposure on " << params.exchName[res.idExchLong] << ": "
                            << volumeLong << std::setprecision(2) << std::endl;
                    logFile << std::setprecision(6) << "BTC exposure on " << params.exchName[res.idExchShort] << ": "
                            << volumeShort << std::setprecision(2) << std::endl;
                    logFile << std::endl;
                    longOrderId = sendLongOrder[res.idExchLong](params, "sell", fabs(btcUsed[res.idExchLong]),
                                                                limPriceLong);
                    shortOrderId = sendShortOrder[res.idExchShort](params, "buy", fabs(btcUsed[res.idExchShort]),
                                                                   limPriceShort);
                    logFile << "Waiting for the two orders to be filled..." << std::endl;
                    sleep(5.0);
                    bool isLongOrderComplete = isOrderComplete[res.idExchLong](params, longOrderId);
                    ...
                    for (int i = 0; i < numExch; ++i) {
                        ...
                        logFile << balance[i].usdAfter << " USD (perf $" << balance[i].usdAfter - balance[i].usd
                                << "), ";
                        logFile << std::setprecision(6) << balance[i].btcAfter << std::setprecision(2) << " BTC"
                                << std::endl;
                    }
                    logFile << std::endl;
                    ...
                    logFile << "ACTUAL PERFORMANCE: " << "$" << res.usdTotBalanceAfter - res.usdTotBalanceBefore << " ("
                            << res.actualPerf() * 100.0 << "%)\n" << std::endl;
                    csvFile << res.id << "," << res.exchNameLong << "," << res.exchNameShort << ","
                            << printDateTimeCsv(res.entryTime) << "," << printDateTimeCsv(res.exitTime);
                    csvFile << "," << res.getTradeLengthInMinute() << "," << res.exposure * 2.0 << ","
